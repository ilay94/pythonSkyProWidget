...
def read_json_from_file(filepatch: str) -> list:
    ...
    try:
        ...
        logger.error(f"В процессе чтения файла возникла ошибка:", exc_info = e)
        return []
...
def get_amount_rub(operation: dict) -> float:
    ...
    try:
        if operation["operationAmount"]["currency"]["code"] != "RUB":
            logger.debug(f"Конвертация {operation["operationAmount"]["amount"]} {operation["operationAmount"]["currency"]["code"]} ")
        ...
        logger.error(f"В получения суммы операции возникла ошибка:", exc_info = e)
